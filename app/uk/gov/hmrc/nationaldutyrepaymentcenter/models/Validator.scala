...
object Validator {
  ...
  def apply[T](constraints: Validate[T]*): Validate[T] =
    (entity: T) =>
      constraints
        .foldLeft[Validated[List[String], Unit]](Valid(()))(
          (v, fx) => v.combine(fx(entity).leftMap(le => le.map(e => if (e.contains(" in ")) e else s"$e in $entity")))
        )
  ...
  def checkIfSome[T, E](element: T => Option[E], validator: Validate[E], isValidIfNone: Boolean = true): Validate[T] =
    (entity: T) =>
      element(entity)
        .map(validator)
        .getOrElse(if (isValidIfNone) Valid(()) else Invalid(List("Some value expected but got None")))
  ...
  def checkIfAtLeastOneIsDefined[T](alternatives: Seq[T => Option[Any]], expectations: String): Validate[T] =
    (entity: T) =>
  ...
  def checkIfOnlyOneSetIsDefined[T](alternatives: Seq[Set[T => Option[Any]]], expectations: String): Validate[T] =
    (entity: T) => {
      val definedSetCount =
