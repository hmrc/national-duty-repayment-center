  ...
  "ClaimController" when {
    "POST /create-case" should {
      "return 201 with CaseID as a result if successful PEGA API call" in {
        ...
        createResponse.correlationId should be(correlationId)
        createResponse.result.get.fileTransferResults.size should be(1)
        createResponse.result.get.fileTransferResults.head.httpStatus should be(200)
      ...
      "return 201 with CaseID and fileResults should have error if file upload fails" in {
        ...
        createResponse.correlationId should be(correlationId)
        createResponse.result.get.fileTransferResults.size should be(1)
        createResponse.result.get.fileTransferResults.head.httpStatus should be(409)
